...
std::priority_queue<std::pair<float, uint32_t>> pq_ivf_search(float* base, float* query, size_t nlist, size_t n, size_t vecdim, size_t m, size_t k_pq, size_t top_k, const float* ivf_cb, const float* pq_cb, const uint32_t* offset, const uint32_t* lst, const uint8_t* base_pq) {
    ...
    for(int j = 0; j < m; j++){
        ...
        for(int i = 0; i < k_pq; i += 4){
            ...
            const float* c1 = pq_cb + (j*k_pq + i) * sub_d;
            const float* c2 = pq_cb + (j*k_pq + i + 1) * sub_d;
            const float* c3 = pq_cb + (j*k_pq + i + 2) * sub_d;
            const float* c4 = pq_cb + (j*k_pq + i + 3) * sub_d;
    ...
    while(!q_ivf.empty()){
        uint32_t cluster_idx = q_ivf.top().second;
        q_ivf.pop();
