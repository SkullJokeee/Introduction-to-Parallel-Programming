...
void ivf_mpi_setup(float*& base_ivf, uint32_t*& list_ivf, uint32_t*& offset_ivf, size_t cb_n, size_t ivf_dim, int rank, int nprocs){
    ...
    MPI_Scatterv(base_ivf, sendcounts_base.data(), displs_base.data(), MPI_FLOAT, 
                 local_base_ivf, (int)(local_data_cnt * ivf_dim), MPI_FLOAT, 0, MPI_COMM_WORLD);
...
std::priority_queue<std::pair<float, uint32_t>> ivf_mpi_search(const float* query, size_t cb_n, size_t ivf_dim, size_t k, const float* codebook_ivf, int rank, int nprocs){
    ...
    #pragma omp parallel
    {
        ...
        for(size_t i = cb_start; i < cb_end; i += 4){
            ...
            if(local_q.size() < nprobe){
                local_q.push({d1, (uint32_t)i});
            }else if(d1 < local_q.top().first){
                local_q.pop();
                local_q.push({d1, (uint32_t)i});
            }
            if(local_q.size() < nprobe){
                local_q.push({d2, (uint32_t)(i+1)});
            }else if(d2 < local_q.top().first){
                local_q.pop();
                local_q.push({d2, (uint32_t)(i+1)});
            }
            if(local_q.size() < nprobe){
                local_q.push({d3, (uint32_t)(i+2)});
            }else if(d3 < local_q.top().first){
                local_q.pop();
                local_q.push({d3, (uint32_t)(i+2)});
            }
            if(local_q.size() < nprobe){
                local_q.push({d4, (uint32_t)(i+3)});
            }else if(d4 < local_q.top().first){
                local_q.pop();
                local_q.push({d4, (uint32_t)(i+3)});
            }
        }
        #pragma omp critical
        {
            while(!local_q.empty()){
                if(centroid_q.size() < nprobe){
                    centroid_q.push(local_q.top());
                }else if(local_q.top().first < centroid_q.top().first){
                    centroid_q.pop();
                    centroid_q.push(local_q.top());
                }
                local_q.pop();
    ...
    std::vector<uint32_t> target_clusters;
    while(!centroid_q.empty()){
        ...
        if(global_cid >= local_start_cluster && global_cid < local_start_cluster + local_cluster_count){
            target_clusters.push_back(global_cid);
        }
    }
    #pragma omp parallel
    {
        ...
        #pragma omp for schedule(dynamic, 1) nowait
        for(size_t c_idx = 0; c_idx < target_clusters.size(); c_idx++){
            
            uint32_t global_cid = target_clusters[c_idx];
            ...
            for(uint32_t i = start; i < last; i += 4){
                ...
                for(int j = 0; j < 4; j++){
                    ...
                    if(local_rst_thread.size() < k){
                        local_rst_thread.push({dis, l});
                    }else if(dis < local_rst_thread.top().first){
