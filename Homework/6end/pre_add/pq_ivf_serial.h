...
std::priority_queue<std::pair<float, uint32_t>> pq_ivf_serial_search(float* base, float* query, size_t nlist, size_t n, size_t vecdim, size_t m, size_t k_pq, size_t top_k, const float* ivf_cb, const float* pq_cb, const uint32_t* offset, const uint32_t* lst, const uint8_t* base_pq){
    size_t nprobe = 20;
    ...
    for(size_t j = 0; j < m; ++j){
        ...
        for(size_t i = 0; i < k_pq; ++i){
            const float* center = pq_cb + (j * k_pq + i) * sub_d;
